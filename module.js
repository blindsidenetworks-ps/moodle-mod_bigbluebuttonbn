...
M.mod_bigbluebuttonbn.view_init_control_panel = function (data) {
  ...
  if (data.running) {
    control_panel_div_html = M.mod_bigbluebuttonbn.view_msg_started_at(data.info.startTime) + ' ' +
      M.mod_bigbluebuttonbn.view_msg_attendees_in(data.info.moderatorCount, data.info.participantCount);
  }
...
M.mod_bigbluebuttonbn.view_msg_attendees_in = function (moderators, participants) {
  ...
  if (typeof moderators != 'undefined' && typeof participants != 'undefined') {
    ...
    if (participants > 1) {
      msg_attendees_in += bigbluebuttonbn.locales.session_has_users;
      msg_attendees_in += ' <b>' + moderators + '</b> ';
      msg_attendees_in += (moderators == 1 ? bigbluebuttonbn.locales.moderator : bigbluebuttonbn.locales.moderators) + ' and ';
      msg_attendees_in += '<b>' + viewers + '</b> ';
      msg_attendees_in += (viewers == 1 ? bigbluebuttonbn.locales.viewer : bigbluebuttonbn.locales.viewers) + '.';
    } else {
      if (viewers > 0) {
...
M.mod_bigbluebuttonbn.view_init_join_button = function (status) {
  ...
  if (status.can_join) {
    Y.DOM.setAttribute(join_button_input, 'onclick',
      'M.mod_bigbluebuttonbn.broker_joinNow(\'' + status.join_url + '\', \'' +
      bigbluebuttonbn.locales.in_progress + '\', ' + status.can_tag + ');');
  } else {
...
M.mod_bigbluebuttonbn.broker_waitModerator = function () {
  ...
  bigbluebuttonbn_ping_interval_id = bigbluebuttonbn_dataSource.setInterval(bigbluebuttonbn.ping_interval, {
    request: "action=meeting_info&id=" + bigbluebuttonbn.meetingid + "&bigbluebuttonbn=" +
        ...
        success: function (e) {
          if (e.data.status) {
            ...
            if (e.data.links === 0) {
              ...
              if (e.data.links == 1) {
                confirmation_warning = bigbluebuttonbn.locales[action +
                  "_confirmation_warning_s"].replace("{$a}", e.data.links) + '. ';
              } else {
                confirmation_warning = bigbluebuttonbn.locales[action +
                  "_confirmation_warning_p"].replace("{$a}", e.data.links) + '. ';
              }
              var recording_type = is_imported_link ?
                bigbluebuttonbn.locales.recording_link : bigbluebuttonbn.locales.recording;
...
M.mod_bigbluebuttonbn.broker_manageRecording = function (action, recordingid, meetingid) {
  ...
  M.mod_bigbluebuttonbn.broker_actionVerification(action, recordingid, meetingid, function (data) {
    /* global console */
    console.info('callback actionVertification');
    if (data.confirmed) {
      bigbluebuttonbn_dataSource.sendRequest({
