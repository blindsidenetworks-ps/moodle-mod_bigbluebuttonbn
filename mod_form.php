...
class mod_bigbluebuttonbn_mod_form extends moodleform_mod
{
    public function definition()
    {
        ...
        if (!$course) {
            $cm = get_coursemodule_from_id('bigbluebuttonbn', optional_param('update', 0, PARAM_INT), 0, false, MUST_EXIST);
        ...
        bigbluebuttonbn_mform_add_block_participants($mform, $cfg, [
            ...
            'users' => $users, 'participant_selection' => $participant_selection, 'participant_list' => $participant_list,
          ]);
    ...
    public function bigbluebuttonbn_mform_add_block_room_room($mform, $cfg)
    {
        ...
        bigbluebuttonbn_mform_add_element($mform, $field['type'], $field['name'], $field['data_type'], $field['description_key'], $cfg['recording_tagging_default']);
    }
    ...
    public function bigbluebuttonbn_mform_add_block_room_recordings($mform, $cfg)
    {
        ...
        bigbluebuttonbn_mform_add_element($mform, $field['type'], $field['name'], $field['data_type'], $field['description_key'], $cfg['recordings_deleted_activities_default']);
    }

    public function bigbluebuttonbn_mform_add_block_room($mform, $cfg)
    {
        if ($cfg['voicebridge_editable'] || $cfg['waitformoderator_editable'] || $cfg['userlimit_editable'] || $cfg['recording_editable'] || $cfg['recording_tagging_editable']) {
    ...
    public function bigbluebuttonbn_mform_add_block_participants($mform, $cfg, $data)
    {
        ...
        $html_participant_selection = html_writer::tag('div',
            html_writer::select($participant_selection['type_options'], 'bigbluebuttonbn_participant_selection_type', $participant_selection['type_selected'], array(), array('id' => 'bigbluebuttonbn_participant_selection_type', 'onchange' => 'M.mod_bigbluebuttonbn.mod_form_participant_selection_set(); return 0;')).
            '&nbsp;&nbsp;'.
            html_writer::select($participant_selection['options'], 'bigbluebuttonbn_participant_selection', $participant_selection['selected'], array(), array('id' => 'bigbluebuttonbn_participant_selection', 'disabled' => 'disabled')).
            '&nbsp;&nbsp;'.
        ...
        $mform->addElement('static', 'my_add_participant', get_string('mod_form_field_participant_add', 'bigbluebuttonbn'), $html_participant_selection);
        $mform->addElement('html', "\n\n");
    ...
    public function bigbluebuttonbn_get_cfg_options()
    {
        $cfg = [
